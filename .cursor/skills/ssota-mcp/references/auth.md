...
SSOTA Plugin supports two authentication paths: local smoke auth for dogfood and OAuth for hosted deployments.
...
Bootstrap from the SSOTA monorepo root:
...
The smoke account is seeded by SSOTA test setup. Do not copy its access token into committed files. Obtain a fresh token when running a local MCP smoke flow.
...
Hosted SSOTA MCP should use OAuth rather than smoke credentials.

Expected hosted flow:

1. The agent client connects to the hosted SSOTA MCP URL.
2. The user authorizes through Supabase OAuth.
3. The client sends the bearer token to the MCP server.
4. SSOTA maps the token subject to the executor identity.
