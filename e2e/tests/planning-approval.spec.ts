...
import { getSmokeInitiativeId, resetSmokeApprovalFixtures } from "../helpers/graph-seed";
...
test.describe("Planning ApprovalInbox", () => {
  ...
  test("stories approval tab marks smoke story ready", async ({ page }) => {
    const initiativeId = await getSmokeInitiativeId();
    const storiesPageId = await getSmokePageIdBySlug(
      "tpl/initiative/planning/stories",
    );
    await loginAsSmoke(page);
    await gotoProject(page, `n/${initiativeId}/p/${storiesPageId}`);

    await page.getByRole("tab", { name: "Approval" }).click();
    const main = page.getByRole("main");
    await expect(main.getByText("Smoke story")).toBeVisible();
    await main.getByRole("button", { name: "Ready" }).click();
    await expect(main.getByText("모두 처리됨")).toBeVisible({ timeout: 15_000 });
  });
});
