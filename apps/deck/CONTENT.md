...
| 7 | Moat | 개발 팀마다 워크플로우가 다르다 | Moat (개발 도메인 맞춤형 워크플로우) |
...
## 7. Moat — "개발 팀마다 워크플로우가 다르다"

> 요구사항·설계·테스트·배포 순서가 팀마다 다르다 — 고정 SaaS 화면으로는 맞출 수 없다 (`PageRenderRow`)

**개발 도메인 — 팀마다 다른 과정 (예시)**

| 팀 | 과정 |
|---|---|
| 의료 AI 개발팀 | 요구사항 → 모델·제품 스펙 → 임상 검증 → 배포 |
| 프로덕트 팀 | OKR → 이니셔티브 → 설계·구현 → 배포 |
| 외주·에이전시 | 스코프 → 태스크 → 리뷰 → 인도 |

**맞춤형이 해자인 이유**

1. **팀 절차를 그대로 반영** — 순서와 산출물이 팀마다 다르다. 고정 화면으로는 맞출 수 없다.
2. **화면·워크플로우를 맞춤 조립** — 문서·테이블·작업대를 팀 절차에 맞게 조합. 개발 도메인 전용 콘솔을 팀별로 만든다.
3. **에이전트도 같은 절차로** — 사람이 정한 워크플로우 위에서 에이전트가 일한다. 맥락과 화면·절차가 한 세트.

**클로징:** 개발 도메인에서 먼저 **맞춤형 무인 운영**을 증명한다 — 이후 다른 지식노동으로 확장 (→ 10. Roadmap).
