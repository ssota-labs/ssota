...
export function ScheduleDialog({
  ...
  // Advanced mode is auto-selected when an existing cron can't be parsed back.
  ...
  const preview = useMemo(() => {
    ...
    if (cronError) return { error: cronError, summary: "", next: null as Date | null };
    return {
      error: null,
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-lg">
        <form onSubmit={handleSubmit}>
          <DialogHeader>
            <DialogTitle>{isEdit ? "Edit schedule" : "Add trigger"}</DialogTitle>
            <DialogDescription>
              Run an agent on a recurring schedule. Outside the chosen window the
              agent never runs, so no tokens are spent.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="grid gap-4 py-4">
            <div className="grid gap-2">
              ...
              <Select
                value={instructionId}
                onValueChange={(value) => value && setInstructionId(value)}
                disabled={isPending}
          ...
          <DialogFooter>
            <Button type="submit" disabled={isPending || Boolean(preview.error)}>
