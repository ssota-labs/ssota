...
export function CronScheduleForm({
  ...
  const initialAdvanced =
    cronToRecurrence(value.cronExpression, value.timezone) === null;
  ...
  const [rawCron, setRawCron] = useState(value.cronExpression);
  ...
  useEffect(() => {
    ...
    setRawCron(value.cronExpression);
    setEnabled(value.enabled);
  }, [value, initialAdvanced, initialRecurrence]);
  ...
  const form = (
    ...
    >
      ...
      {advanced && (
        <div className="grid gap-2">
          <Label htmlFor={`${formId}-raw-cron`}>Cron expression</Label>
          <Input
            id={`${formId}-raw-cron`}
            value={rawCron}
