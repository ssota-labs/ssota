...
function acceptanceCoversTimelineDurationEdit(
  entry: (typeof starterAcceptance)[number],
): boolean {
  return (
...
function schemaHasAnimatedProductOutput(): boolean {
  if (starterSchema.panels.timeline?.enabled) {
    return true;
  }

  if (
    starterTransferMode.animationIntent?.mode === "autonomous" ||
    starterTransferMode.animationIntent?.mode === "timeline-keyframes" ||
    starterTransferMode.animationIntent?.mode === "timeline-playback"
  ) {
    return true;
  }
  ...
  if (starterTransferMode.referenceTimeline.mode !== "none") {
    return true;
  }

  return starterTransferMode.behaviorCoverage.some((coverage) =>
    [
      "renderer-loop",
      "pause-resume",
      "restart",
...
describe("Toolcraft template app acceptance coverage", () => {
  ...
  it("does not allow app-level playback transport beside the runtime timeline", () => {
    ...
    const referenceTimelineMode =
      starterTransferMode.mode === "reference-runtime-clone"
        ? starterTransferMode.referenceTimeline.mode
        : null;

    if (
      starterSchema.panels.timeline?.enabled &&
  ...
  it("publishes control order targets for app schema tests", () => {
    expect(getToolcraftControlOrderTargets(starterSchema)).toEqual([]);
  });
  ...
  it("requires autonomous animation intent when animation controls exist without a timeline", () => {
    ...
    expect(
      ...
      'Animation controls "Animation / speed" (animation.speed) exist while panels.timeline is omitted. Use panels.timeline mode "playback" for product animation transport, mode "keyframes" for editable keyframes, or declare starterTransferMode.animationIntent mode "autonomous" with coverage proving there is no user-facing transport.',
    );
  });
  ...
  it("requires layer behavior coverage when a layers panel is enabled", () => {
    ...
    expect(
      validateToolcraftAcceptanceCoverage(layersSchema, starterAcceptance),
    ).toEqual(
      expect.arrayContaining([
