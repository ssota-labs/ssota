...
describe("adapter-supabase integration", () => {
  ...
  it.skipIf(() => skip)("smoke 계정 인증 성공", () => {
    expect(smokeUserId).toBeTruthy();
  });

  it.skipIf(() => skip)("create_note 커밋 + action_log 기록", async () => {
  ...
  it.skipIf(() => skip)("비기록 변경 0건: commit은 항상 logEntry와 함께", async () => {
  ...
  it.skipIf(() => skip)("promote_document는 Agent 호출 시 게이트 큐", async () => {
