...
export function StoryCatalog({
  ...
  const groups = useMemo(() => groupStoriesByTitle(stories), [stories]);
  ...
  const filteredGroups = useMemo(() => {
    ...
    for (const [title, storyList] of groups) {
      const matched = storyList.filter(
        (s) =>
  ...
  return (
    <div className="flex h-full min-h-0 flex-col border-r border-border bg-card">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto p-2">
        {[...filteredGroups.entries()].map(([title, storyList]) => (
