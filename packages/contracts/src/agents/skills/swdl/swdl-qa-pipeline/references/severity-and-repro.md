...
3. Leave the `pull_request` at `approved` (review outcome stands); it must not merge until the test_plan reaches `pass` — that block is convention, not a status flip.
