...
/** Normalize persisted workflow trigger events (legacy string[] or structured). */
export function normalizeWorkflowTriggerEvents(
  raw: unknown,
): WorkflowTriggerEvent[] {
...
/** Normalize legacy trigger specs (patterns-only or string events). */
export function normalizeWorkflowTriggerSpec(raw: unknown): WorkflowTriggerSpec {
  if (raw && typeof raw === "object") {
    const value = raw as Record<string, unknown>;
    if (Array.isArray(value.events) && value.events.length > 0) {
      return { events: normalizeWorkflowTriggerEvents(value.events) };
    }
    if (Array.isArray(value.patterns) && value.patterns.length > 0) {
      return {
        events: value.patterns.map((kind, index) => {
          const kindStr = String(kind);
          return {
            id: kindStr === "manual" ? "manual" : `legacy_${index}_${kindStr}`,
            kind: kindStr,
            enabled: true,
            config: {},
          };
  ...
  return { events: [createManualWorkflowTrigger()] };
}
