...
- **Root Runtime Protocol**은 `loopos-mcp` 스킬이 담당한다. 배포 번들 SSOT는 `plugins/loopos-plugin/skills/loopos-mcp`이며, 워크스페이스 복제본은 `.agents/skills/loopos-mcp`·`.cursor/skills/loopos-mcp`·`.cursor/mcp.json`에 있다. 플러그인 수정 시 같은 PR에서 복제본도 함께 갱신한다. 그래프의 Instructions는 **도메인 레시피**만 저장한다.
