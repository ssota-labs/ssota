...
데이터 접근 패턴(준수 대상): **Server Action → `executeAction(ports, …)` → adapter ports → Supabase(Drizzle)**, 이후 `revalidatePath`. 예: `apps/web/app/actions.ts` 의 `updateTaskStatusAction`. 에디터 저장도 이 흐름을 따른다(직접 supabase mutate 금지).
