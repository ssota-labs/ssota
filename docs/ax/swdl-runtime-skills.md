...
> Main 에이전트는 없다. 대상은 Research / Planning / Delivery / QA / Orchestrator 다섯 명뿐이다.  
...
| `swdl-handoff` | Package outputs so the next SWDL specialist can continue without re-discovery. Use before marking done when another role will consume the result. | handoff 필드 최소셋 | `handoff-schema.md` (research→planning→delivery→qa 슬롯) |

### 전문 (역할당 1)

| id | owner | description | references/ |
|----|-------|-------------|-------------|
| `swdl-research-intake` | Research | Turn a research ask into `research` / `finding` / `insight` (and related) nodes linked to the initiative. Use for discovery, competitive, user, or market research work orders. | `types-and-edges.md`, `pages.md`, `quality-bar.md` |
| `swdl-planning-intake` | Planning | Turn insights/goals into `prd` / `user_story` / `feature` / roadmap structure under an initiative. Use for product planning and scope definition. | `types-and-edges.md`, `pages.md`, `splitting-rules.md` |
| `swdl-delivery-intake` | Delivery | Turn planned scope into `implementation_plan` / `task` / `pull_request` / sprint execution updates. Use for build, board, and PR tracking work orders. | `types-and-edges.md`, `pages.md`, `task-lifecycle.md` |
| `swdl-qa-intake` | QA | Turn delivery candidates into `test_plan` / `test_case` / `bug` / verification outcomes. Use for QA, regression, and acceptance work orders. | `types-and-edges.md`, `pages.md`, `severity-and-repro.md` |

### 오케스트레이션 (1)

| id | owner | description | references/ |
|----|-------|-------------|-------------|
| `swdl-orchestrate` | Orchestrator | Triage open work orders and route to Research, Planning, Delivery, or QA; keep schedule-driven sweeps honest. Use on every orchestrator run — do not perform specialist graph authoring yourself. | `routing-table.md`, `schedule-policy.md`, `escalation.md` |

Orchestrator는 **전문 파이프라인 스킬을 장착하지 않는다.** 라우팅 테이블만 알면 된다 (progressive: 상세 타입 스키마는 specialist가 연다).
...
| Agent | 장착 스킬 |
|-------|-----------|
| **Research** | `swdl-graph-ops`, `swdl-task-contract`, `swdl-handoff`, `swdl-research-intake` |
...
2. 전문 4 pipeline 스킬 — 현재 실제 존재하는 타입/페이지만 ref에 기입
