...
| `research/market` | `Tabs` → Studies `DocumentCardListSheet` / Competitors·Segments `Resizable`+`DataTable`+`DocumentEditor` / Sources `MediaEmbed`+`DocumentEditor` | `Tabs` 4-panel hub + `competitor`/`market_segment`/`raw_source` L1 | hub + structured entities | ⏳ |
...
research                           PageHeader
research/market                    Tabs → Studies(DocumentCardListSheet) + Competitors/Segments/Sources(DataTable+DocumentEditor, MediaEmbed)  [Δ]
